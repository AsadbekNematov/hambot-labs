...
Purpose:
    PID-based forward wall stop using HamBot LiDAR. The robot advances toward a wall
    and stops at a target clearance (default 1.0 m).
    """
import argparse
import math
import os
...
def _front_distance_m(range_image: Iterable[float],
    ...
    """Return the averaged forward distance (meters) from the LiDAR around FRONT_INDEX.
Uses a ±FRONT_WINDOW sample window and clamps noisy/invalid readings."""
...
def forward_wall_stop(bot: HamBot,
    ...
    """Run a PID control loop to approach a wall and stop at TARGET_DIST_M.
Maintains a fixed loop cadence, clamps motor RPM, and declares success when
the distance remains within DEFAULT_SETTLE_BAND for DEFAULT_SETTLE_TIME seconds."""
    """
...
def main() -> None:
    """Parse CLI arguments, build HamBot and PID configuration, and execute the demo.
This function is the entry point used for submission and testing."""
